...
// Export singleton instance (v2 style)
export const db = new KVDatabase("AskArchitect", "state");

/**
 * Global state selector (v2 style)
 */
if (!window.state) {
    window.state = {
        selector: (selectorStr) => {
            const STATE_PREFIX = "ask_arch_state_";
            const [tagName, id] = selectorStr.includes("#") 
                ? selectorStr.split("#") 
                : [selectorStr, "global"];
            
            const keyBase = `${STATE_PREFIX}${tagName.toLowerCase()}#${id}.`;
            
            return new Proxy({}, {
                get: (target, attr) => db.get(keyBase + attr)
            });
        }
    };
}
