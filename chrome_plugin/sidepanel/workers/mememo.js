...
class NodesInIndexedDB {
     ...
     */
    constructor(graphLayers, shouldPreComputeDistance, prefetchSize, distanceCacheMaxSize = 4096) {
        this.nodesMap = new Map();
    ...
    async keys() {
        const db = await this.dbPromise;
        const results = (await db.toCollection().primaryKeys());
        return results;
    }
    async bulkSet(keys, values) {
     ...
     */
    _updateAutoPrefetchSize(embeddingDim) {
        if (!this.hasSetPrefetchSize) {
     ...
     */
    async _prefetch(key, level) {
        this.nodesMap.clear();
        // BFS traverse the current graph
        const graphLayer = this.graphLayers[level];
        const nodeCandidateCompare = (candidate) => candidate.distance;
...
export class HNSW {
     ...
     */
    constructor({ distanceFunction, m, efConstruction, mMax0, ml, seed, useIndexedDB, distancePrecision }) {
        ...
        if (useIndexedDB === undefined || useIndexedDB === false) {
            ...
            this.nodes = new NodesInIndexedDB(this.graphLayers, this.useDistanceCache);
        }
        // Set the distance function which has access to the distance cache
        this.distanceFunction = (a, b, aKey, bKey) => {
     ...
     */
    exportIndex() {
        const graphLayers = this.graphLayers.map(d => d.toJSON());
        const mememoIndex = {
     ...
     */
    async insert(key, value, maxLevel) {
        ...
        if (await this.nodes.has(key)) {
            ...
            throw Error(`There is already a node with key ${key} in the` +
     ...
     * this value in most cases. We add this parameter for testing purpose.
     */
    async bulkInsert(keys, values, maxLevels) {
     ...
     * @param value The embedding of the new element to insert.
     */
    async insertSkipIndex(key, value) {
        // If the key already exists, throw an error
        if (await this.nodes.has(key)) {
     ...
     * @param values The embeddings of the new elements to insert.
     */
    async bulkInsertSkipIndex(keys, values) {
        // If the key already exists, throw an error
        const existingKeys = await this.nodes.keys();
        for (const key of keys) {
            if (existingKeys.includes(key)) {
     ...
     */
    async _insertToGraph(key, value, level) {
        if (this.entryPointKey !== null) {
            ...
            const entryPointInfo = await this._getNodeInfo(this.entryPointKey, this.graphLayers.length - 1);
            // Start with the entry point
            let minDistance = this.distanceFunction(value, entryPointInfo.value, key, entryPointInfo.key);
            ...
            for (let l = this.graphLayers.length - 1; l >= level + 1; l--) {
                const result = await this._searchLayerEF1(key, value, minNodeKey, minDistance, l);
            ...
            for (let l = nodeHightLayerLevel; l >= 0; l--) {
                ...
                entryPoints = await this._searchLayer(key, value, entryPoints, l, this.efConstruction);
                // Prune the neighbors so we have at most levelM neighbors
                const selectedNeighbors = await this._selectNeighborsHeuristic(entryPoints, levelM, l);
                // Insert the new node
                ...
                for (const neighbor of selectedNeighbors) {
                    ...
                    const neighborNeighborCandidates = [];
                    for (const [key, distance] of neighborNode.entries()) {
                        const candidate = { key, distance };
                    ...
                    const selectedNeighborNeighbors = await this._selectNeighborsHeuristic(neighborNeighborCandidates, levelM, l);
                    // Update this neighbor's neighborhood
                    const newNeighborNode = new Map();
    ...
    async update(key, value) {
        ...
        for (let l = 0; l < this.graphLayers.length; l++) {
            ...
            const secondDegreeNeighborhood = new Set([key]);
            ...
            for (const firstDegreeNeighbor of curNode.keys()) {
                ...
                const firstDegreeNeighborNode = curGraphLayer.graph.get(firstDegreeNeighbor);
            ...
            const nodeCompare = (candidate) => candidate.distance;
            for (const firstDegreeNeighbor of curNode.keys()) {
                // (1) Find `efConstruction` number of candidates
                const candidateMaxHeap = new MaxHeap(nodeCompare);
                const firstDegreeNeighborInfo = await this._getNodeInfo(firstDegreeNeighbor, l);
                for (const secondDegreeNeighbor of secondDegreeNeighborhood) {
                    if (secondDegreeNeighbor === firstDegreeNeighbor) {
                        continue;
                    }
                    const secondDegreeNeighborInfo = await this._getNodeInfo(secondDegreeNeighbor, l);
                    const distance = this.distanceFunction(firstDegreeNeighborInfo.value, secondDegreeNeighborInfo.value, firstDegreeNeighborInfo.key, secondDegreeNeighborInfo.key);
                ...
                const selectedCandidates = await this._selectNeighborsHeuristic(candidates, levelM, l);
     ...
     */
    async query(value, k = undefined, ef = this.efConstruction) {
        if (this.entryPointKey === null) {
            throw Error('Index is not initialized yet');
        }
        // EF=1 search from the top layer to layer 1
        let minNodeKey = this.entryPointKey;
        const entryPointInfo = await this._getNodeInfo(minNodeKey, this.graphLayers.length - 1);
        let minNodeDistance = this.distanceFunction(entryPointInfo.value, value, null, null);
        for (let l = this.graphLayers.length - 1; l >= 1; l--) {
            const result = await this._searchLayerEF1(null, value, minNodeKey, minNodeDistance, l, false);
            minNodeKey = result.minNodeKey;
        ...
        const candidates = await this._searchLayer(null, value, entryPoints, 0, ef, false);
     ...
     */
    async _reIndexNode(key, value) {
        if (this.entryPointKey === null) {
            throw Error('entryPointKey is null');
        }
        let minNodeKey = this.entryPointKey;
        const entryPointInfo = await this._getNodeInfo(minNodeKey, this.graphLayers.length - 1);
        let minNodeDistance = this.distanceFunction(entryPointInfo.value, value, entryPointInfo.key, key);
        let entryPoints = [
            { key: minNodeKey, distance: minNodeDistance }
        ];
        ...
        for (let l = this.graphLayers.length - 1; l >= 0; l--) {
            ...
            if (!curGraphLayer.graph.has(key)) {
                ...
                const result = await this._searchLayerEF1(key, value, minNodeKey, minNodeDistance, l);
                minNodeKey = result.minNodeKey;
                minNodeDistance = result.minDistance;
            }
            else {
                ...
                const levelM = l === 0 ? this.mMax0 : this.m;
                // Search for closest points at this level to connect with
                entryPoints = await this._searchLayer(key, value, entryPoints, l, 
     ...
     */
    async _searchLayerEF1(queryKey, queryValue, entryPointKey, entryPointDistance, level, canReturnDeletedNode = true) {
        const graphLayer = this.graphLayers[level];
        const nodeCandidateCompare = (candidate) => candidate.distance;
        const candidateHeap = new MinHeap(nodeCandidateCompare);
        ...
        while (candidateHeap.size() > 0) {
            ...
            for (const key of curNode.keys()) {
                if (!visitedNodes.has(key)) {
                    ...
                    const distance = this.distanceFunction(curNodeInfo.value, queryValue, curNodeInfo.key, queryKey);
     ...
     * @param canReturnDeletedNodes Whether to return deleted nodes
     */
    async _searchLayer(queryKey, queryValue, entryPoints, level, ef, canReturnDeletedNodes = true) {
        const graphLayer = this.graphLayers[level];
        ...
        while (candidateMinHeap.size() > 0) {
            ...
            for (const neighborKey of curNode.keys()) {
                if (!visitedNodes.has(neighborKey)) {
                    ...
                    const distance = this.distanceFunction(queryValue, neighborInfo.value, queryKey, neighborInfo.key);
                    const furthestFoundNode = foundNodesMaxHeap.root();
                    // Add this node if it is better than our found nodes or we do not
                    // have enough found nodes
                    if (distance < furthestFoundNode.distance ||
     ...
     * @param level Current graph layer level
     */
    async _selectNeighborsHeuristic(candidates, maxSize, level) {
        ...
        while (candidateMinHeap.size() > 0) {
            ...
            for (const selectedNeighbor of selectedNeighbors) {
                ...
                const neighborInfo = await this._getNodeInfo(selectedNeighbor.key, level);
                const distanceCandidateToNeighbor = this.distanceFunction(candidateInfo.value, neighborInfo.value, candidate.key, neighborInfo.key);
...
//# sourceMappingURL=mememo.js.map